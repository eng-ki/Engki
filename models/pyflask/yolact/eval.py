...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45):
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        colors = torch.cat([torch.FloatTensor(get_color(j, on_gpu=img.device.index)).view(
...
def prep_benchmark(dets_out, h, w):
    ...
    with timer.env('Copy'):
        classes, scores, boxes, masks = [
            x[:args.top_k].cpu().numpy() for x in t]
        ...
        with timer.env('JSON Output'):
            ...
            for i in range(masks.shape[0]):
                ...
                if (boxes[i, 3] - boxes[i, 1]) * (boxes[i, 2] - boxes[i, 0]) > 0:
                    detections.add_bbox(
                        image_id, classes[i], boxes[i, :],   scores[i])
                    detections.add_mask(
                        image_id, classes[i], masks[i, :, :], scores[i])
            return
...
def evalvideo(net: Yolact, path: str):
    ...
    def extract_frame(x, i): return (
        x[0][i] if x[1][i] is None else x[0][i].to(x[1][i]['box'].device), [x[1][i]])
        ...
        for frame in list(reversed(active_frames)):
            ...
            if frame['idx'] == 0:
                ...
                active_frames += [{'value': extract_frame(
                    frame['value'], i), 'idx': 0} for i in range(1, args.video_multiframe)]
                frame['value'] = extract_frame(frame['value'], 0)

        # Finish loading in the next frames and add them to the processing queue
        active_frames.append(
            {'value': next_frames.get(), 'idx': len(sequence)-1})
    ...
    try:
        for i in range(num_frames):
            ...
            with timer.env('Video'):
                ...
                preds = net(batch)
                processed = prep_display(
                    preds, frame, None, None, undo_transform=False, class_color=True)
