...
from yolact2.data import cfg, mask_type
...
class Detect(object):
    ...
    def __call__(self, predictions, net):
        ...
        with timer.env('Detect'):
            ...
            for batch_idx in range(batch_size):
                ...
                result = self.detect(batch_idx, conf_preds,
                                     decoded_boxes, mask_data, inst_data)
    ...
    def fast_nms(self, boxes, masks, scores, iou_threshold: float = 0.5, top_k: int = 200, second_threshold: bool = False):
        ...
        classes = torch.arange(num_classes, device=boxes.device)[
            :, None].expand_as(keep)
    ...
    def traditional_nms(self, boxes, masks, scores, iou_threshold=0.5, conf_thresh=0.05):
        ...
        for _cls in range(num_classes):
            ...
            preds = torch.cat(
                [boxes[conf_mask], cls_scores[:, None]], dim=1).cpu().numpy()
