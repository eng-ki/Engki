...
from yolact2.data import cfg, mask_type, activation_func
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, net, predictions, targets, masks, num_crowds):
        ...
        for idx in range(batch_size):
            ...
            if cfg.use_class_existence_loss:
                ...
                class_existence_t[idx, :] = torch.eye(
                    num_classes-1, device=conf_t.get_device())[labels[idx]].max(dim=0)[0]

            # Split the crowd annotations because they come bundled in
            cur_crowds = num_crowds[idx]
            if cur_crowds > 0:
                def split(x): return (x[-cur_crowds:], x[:-cur_crowds])
        ...
        if cfg.train_boxes:
            ...
            losses['B'] = F.smooth_l1_loss(
                loc_p, loc_t, reduction='sum') * cfg.bbox_alpha
            ...
            if cfg.mask_type == mask_type.direct:
                if cfg.use_gt_bboxes:
                    ...
                    losses['M'] = F.binary_cross_entropy(torch.clamp(
                        masks_p, 0, 1), masks_t, reduction='sum') * cfg.mask_alpha
                else:
                    losses['M'] = self.direct_mask_loss(
                        pos_idx, idx_t, loc_data, mask_data, priors, masks)
            elif cfg.mask_type == mask_type.lincomb:
                ret = self.lincomb_mask_loss(
                    pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels)
                if cfg.use_maskiou:
                ...
                if cfg.mask_proto_loss is not None:
                    if cfg.mask_proto_loss == 'l1':
                        losses['P'] = torch.mean(
                            torch.abs(proto_data)) / self.l1_expected_area * self.l1_alpha
                    elif cfg.mask_proto_loss == 'disj':
                        losses['P'] = - \
                            torch.mean(torch.max(F.log_softmax(
        ...
        if cfg.use_focal_loss:
            ...
            if cfg.use_objectness_score:
                ...
                losses['C'] = self.ohem_conf_loss(
                    conf_data, conf_t, pos, batch_size)
    ...
    def ohem_conf_loss(self, conf_data, conf_t, pos, num):
        ...
        if cfg.use_class_balanced_conf:
            ...
            weighting = 1 - \
                (self.class_instances[targets_weighted] / self.total_instances)
    ...
    def focal_conf_loss(self, conf_data, conf_t):
        ...
        at = (1 - cfg.focal_loss_alpha) * background + \
            cfg.focal_loss_alpha * (1 - background)
    ...
    def focal_conf_sigmoid_loss(self, conf_data, conf_t):
        ...
        # -1 if background, +1 if forground for specific class
        conf_pm_t = conf_one_t * 2 - 1

        # note: 1 - sigmoid(x) = sigmoid(-x)
        logpt = F.logsigmoid(conf_data * conf_pm_t)
        pt = logpt.exp()

        at = cfg.focal_loss_alpha * conf_one_t + \
            (1 - cfg.focal_loss_alpha) * (1 - conf_one_t)
        # Set alpha for the background class to 0 because sigmoid focal loss doesn't use it
        at[..., 0] = 0
    ...
    def focal_conf_objectness_loss(self, conf_data, conf_t):
        ...
        logpt = F.logsigmoid(
            conf_data[:, 0]) * (1 - background) + F.logsigmoid(-conf_data[:, 0]) * background
        pt = logpt.exp()
    ...
    def conf_objectness_loss(self, conf_data, conf_t, batch_size, loc_p, loc_t, priors):
        ...
        obj_pos_loss = - iou_targets * \
            F.logsigmoid(obj_data_pos) - (1 - iou_targets) * \
            F.logsigmoid(-obj_data_pos)
    ...
    def direct_mask_loss(self, pos_idx, idx_t, loc_data, mask_data, priors, masks):
        ...
        for idx in range(mask_data.size(0)):
            with torch.no_grad():
                ...
                mask_t = torch.cat(scaled_masks, 0).gt(
                    0.5).float()  # Threshold downsampled mask

            pos_mask_data = mask_data[idx, cur_pos_idx_squeezed, :]
            loss_m += F.binary_cross_entropy(torch.clamp(
                pos_mask_data, 0, 1), mask_t, reduction='sum') * cfg.mask_alpha
    ...
    def coeff_diversity_loss(self, coeffs, instance_t):
        ...
        inst_eq = (instance_t[:, None].expand_as(cos_sim)
                   == instance_t[None, :].expand_as(cos_sim)).float()
        ...
        for idx in range(mask_data.size(0)):
            with torch.no_grad():
                ...
                if cfg.mask_proto_remove_empty_masks:
                    ...
                    very_small_masks = (
                        downsampled_masks.sum(dim=(0, 1)) <= 0.0001)
                ...
                if cfg.mask_proto_reweight_mask_loss:
                    ...
                    gt_background_norm = (
                        1-bin_gt) / (torch.sum(1-bin_gt, dim=(0, 1), keepdim=True) + 0.0001)

                    mask_reweighting = gt_foreground_norm * \
                        cfg.mask_proto_reweight_coeff + gt_background_norm
            ...
            if process_gt_bboxes:
                ...
                if cfg.mask_proto_crop_with_pred_box:
                    pos_gt_box_t = decode(
                        loc_data[idx, :, :], priors.data, cfg.use_yolo_regressors)[cur_pos]
                else:
            ...
            if cfg.mask_proto_mask_activation == activation_func.sigmoid:
                pre_loss = F.binary_cross_entropy(torch.clamp(
                    pred_masks, 0, 1), mask_t, reduction='none')
            else:
                pre_loss = F.smooth_l1_loss(
                    pred_masks, mask_t, reduction='none')
