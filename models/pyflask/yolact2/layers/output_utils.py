...
from yolact2.data import cfg, mask_type, MEANS, STD, activation_func
from yolact2.utils.augmentations import Resize
...
def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
    ...
    if dets is None:
        # Warning, this is 4 copies of the same thing
    ...
    if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch:
            ...
            with timer.env('maskiou_net'):
                with torch.no_grad():
                    ...
                    maskiou_p = torch.gather(
                        maskiou_p, dim=1, index=classes.unsqueeze(1)).squeeze(1)
                    if cfg.rescore_mask:
                        if cfg.rescore_bbox:
        ...
        masks = F.interpolate(masks.unsqueeze(
            0), (h, w), mode=interpolation_mode, align_corners=False).squeeze(0)
...
def undo_image_transformation(img, w, h):
    ...
    if cfg.backbone.transform.normalize:
        ...
        img_numpy = (img_numpy / 255.0 + np.array(MEANS) /
    ...
    for kdx in range(1):
        ...
        for y in range(arr_h):
            for x in range(arr_w):
                ...
                if i == 0:
                    running_total = proto_data[:, :, idx[i]].cpu(
                    ).numpy() * coeffs_sort[i]
                else:
                    running_total += proto_data[:, :,
                                                idx[i]].cpu().numpy() * coeffs_sort[i]
                ...
                arr_img[y*proto_h:(y+1)*proto_h, x*proto_w:(x+1)*proto_w] = (proto_data[:, :,
                                                                                        idx[i]] / torch.max(proto_data[:, :, idx[i]])).cpu().numpy() * coeffs_sort[i]
                arr_run[y*proto_h:(y+1)*proto_h, x*proto_w:(x+1) *
                        proto_w] = (running_total_nonlin > 0.5).astype(np.float)
        plt.imshow(arr_img)
        plt.show()
        # plt.imshow(arr_run)
        # plt.show()
