...
from yolact2.data.config import cfg, mask_type
from yolact2.layers import Detect
...
class PredictionModule(nn.Module):
        ...
        if parent is None:
            ...
            if cfg.use_instance_coeff:
                self.inst_layer = nn.Conv2d(
                    out_channels, self.num_priors * cfg.num_instance_coeffs, **cfg.head_layer_params)
    ...
    def forward(self, x):
        ...
        if cfg.eval_mask_branch:
            ...
            mask = torch.zeros(x.size(0), bbox.size(
                1), self.mask_dim, device=bbox.device)

        if cfg.use_mask_scoring:
            score = src.score_layer(x).permute(
                0, 2, 3, 1).contiguous().view(x.size(0), -1, 1)
        ...
        with timer.env('makepriors'):
            if self.last_img_size != (cfg._tmp_img_w, cfg._tmp_img_h):
                ...
                self.priors = torch.Tensor(
                    prior_data, device=device).view(-1, 4).detach()
...
class FastMaskIoUNet(ScriptModuleWrapper):
    ...
    def forward(self, x):
        ...
        maskiou_p = F.max_pool2d(x, kernel_size=x.size()[
                                 2:]).squeeze(-1).squeeze(-1)
        ...
        for name, module in self.named_modules():
            ...
            if 'Script' in type(module).__name__:
                ...
                # 1.3 workaround: check if this has the same constants as a conv module
                    ...
                    is_script_conv = (
                        all_in(module.__dict__[
                               '_constants_set'], conv_constants)
        ...
        with timer.env('pred_heads'):
            ...
            for idx, pred_layer in zip(self.selected_layers, self.prediction_layers):
                ...
                if cfg.mask_type == mask_type.lincomb and cfg.mask_proto_prototypes_as_features:
                    ...
                    proto_downsampled = F.interpolate(proto_downsampled, size=outs[idx].size()[
            ...
            if cfg.use_focal_loss:
                if cfg.use_sigmoid_focal_loss:
                    ...
                    pred_outs['conf'][:, :, 1:] = objectness[:, :,
                                                             None] * F.softmax(pred_outs['conf'][:, :, 1:], -1)
